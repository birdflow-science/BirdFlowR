...
# Placeholders so inline `r` expressions below don't error when the
# chunks that normally set these are skipped (BirdFlowModels missing).
if (!has_models) {
  polys <- data.frame(id = NA_character_)
  sel <- 1
  plot_count_threshold <- NA_integer_
}
...
```{r select largest, echo = FALSE, eval = has_models}
