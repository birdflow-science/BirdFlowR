...
## Download and read polygons

Download state boundaries from the U.S. Census, read, and cleanup.
Any set of spatial polygons with a defined CRS would work equally well.
...
# Download and unzip state boundaries
url <- "http://www2.census.gov/geo/tiger/TIGER2012/STATE/tl_2012_us_state.zip"
dir <- tempdir()
shp_dir <- file.path(dir, "shapefile")
dir.create(shp_dir)
local_zip <- file.path(dir, basename(url))
utils::download.file(url, destfile = local_zip)
utils::unzip(local_zip, exdir = shp_dir)
shp_path <- list.files(shp_dir, pattern = "\\.shp$", full.names = TRUE)

# Load state boundaries
# Variable names are generic to make sense with any set of polygons.
polys <- sf::read_sf(shp_path)

# Drop non-contigous
to_drop <- c("Hawaii", "Commonwealth of the Northern Mariana Islands", 
             "Puerto Rico", "United States Virgin Islands", "Alaska", "Guam", 
             "American Samoa")
polys <- polys[!polys$NAME %in% to_drop, ]

# Select columns
polys <- polys[ , c("GEOID", "STUSPS", "NAME") ]
