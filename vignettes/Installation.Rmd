  ...
  Open RStudio and run the following in the console.
   ...
   Package dependencies can be a pain. If the above doesn't work you can also try the alternative method below, also executed in the RStudio console.
    ...
    pak::pkg_install("birdflow-science/BirdFlowR", ask = FALSE,
                     dependencies = TRUE)
    ```
   If you are having issues installing the **rhdf5** package distributed through
   [bioconductor](https://bioconductor.org/) you could try these alternate ways 
   of installing it and then try installing BirdFlowR again.
   
   
    ```{r Install packages with install_bioc, eval=FALSE, include=TRUE}
    devtools::install_bioc("rhdf5")
    ```

    If that still doesn't work try using the bioconductor installer to install **rhdf5**.

    ```{r Install packages with BiocManager, eval=FALSE, include=TRUE}
...
Run these lines to see if you've installed the two BirdFlow packages and their dependencies.
