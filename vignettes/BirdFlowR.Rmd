...
does not use the legacy packages r data formats itself.*
...
the starting timestep so the following is equivalent to the preceeding two sections.

```{r route with sample}
rts <- route(bf,  n = n_positions,  start = start, end = end )
...
```{r route with season}
rts <- route(bf, n = n_positions, season = "prebreeding")
```
