...
A BirdFlow model is built on a raster grid with a **static mask** that defines
...
active cell, representing a probability mass function: values sum to
approximately 1 and each gives the proportion of the population in that cell.
Multiple distributions are stored as a matrix with `n_active(bf)` rows and one
column per timestep — the common format returned by `get_distr()` and consumed by
`predict()`.
...
BirdFlowR uses the **terra** package for raster data and provides BirdFlow
methods for terra functions, so you can use them directly on BirdFlow objects.
...
anchored to a nominal year. There is an explicit link forward in time from
