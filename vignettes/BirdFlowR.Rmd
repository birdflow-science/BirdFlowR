...
`n_transitions()` and `n_distr()` report on temporal dimensions. If the model is circular, they will be equal; for temporally linear models, `n_distr()` will be one more than `n_transitions()`.
...
Alternatively convert directly from BirdFlow to SpatRast with `rast()`. The second (optional) argument `which` accepts the same inputs as `which` in `get_distr()`.
...
BirdFlowR provides some convenience wrappers to functions in rnaturalearth that load vector data and then crop and transform it to make it suitable for plotting with BirdFlow output.
...
Additionally, we can calculate the difference between the projected distribution and the distribution of the 
...
First extract the winter distribution, then use `sample_locations()` with `n = n_positions` to sample the input distribution repeatedly. The result is a matrix in which each column has a single '1' representing the sampled location.
...
## Simplified route() work flow

`route()` will sample the eBirdstatus and trends distribution for the starting
timestep to generate the locations if `x_coord` and `y_coord` are NULL. 

Additionally, time can be specified with the `season` argument 
possibly along with `season_buffer`. This uses eBird Status and Trends metadata
stored in the BirdFlow object to determine the start and ending dates.

The code below generates 10 routes for the prebreeding migration season using 
the default season buffer of 1 week - extending 1 week before and after the 
named season.
...
`plot_routes()` will visualize time as a color gradient using **ggplot2**. 
