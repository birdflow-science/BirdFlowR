...
BirdFlow models represent time as a loop of 52 timesteps (weeks), loosely
anchored to a nominal year. There is an explicit link from
timestep 52 to timestep 1.

```{r time-structure-figure, echo = FALSE, out.width = "80%", fig.align = "center", fig.cap = "BirdFlow's 52-timestep year. Dots mark timesteps; lines between them mark transitions. Colors match the seasonal gradient used by plot_routes(). The bold line at the Dec/Jan boundary marks the year wrap-around. Season ranges shown are for American Woodcock."}
...
The functions `predict()`, `route()`, and others accept `start` and `end`
arguments that are forwarded to `lookup_timestep_sequence()`, which builds the
...
Although it's generally not called directly, we'll demonstrate the time
...
They are returned by `get_distr()` and both consumed and returned by `predict()`.
...
of the eBird Status and Trends data the model was trained on. 

A **distribution** is a numeric vector of length `n_active(bf)`, one value per
active cell.  Distributions usually represent a probability mass function: values sum to 1 and each gives
the proportion of the population in the corresponding cell.
Multiple distributions are stored as a matrix with `n_active(bf)` rows and one
column per timestep.
Distributions are model-specific: each BirdFlow model has its own extent and
...
r <- rasterize_distr(d, bf) # convert to SpatRaster (terra package data format)
```
...
We can calculate the difference between the projected distribution
