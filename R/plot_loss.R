...
#' loss which has three components.
#'
...
#' * Distance loss is lower when the routes encoded in the model are shorter.
#' * Entropy loss is lower when the entropy in the model is higher.
#'
#' The total loss is the weighted sum of the three component losses. The
#' weighting may cause it to be lower than some of the components.
#'
#' @param bf A fitted BirdFlowModel
...
plot_loss <- function(bf, transform = "log10") {
  ...
  d <- tidyr::pivot_longer(loss, cols = -ncol(loss), values_to = "Loss", names_to = "Type")
  ...
  ggplot(data = d, aes(x = Step, y = Loss, color = Type)) +
