...
#' data in `x`. It's used in BirdFlowR to preprocess data.
#'
#' `make_mask` is a private function.
#'
#' @param x A SpatRaster, typically it's multilayered and contains the
#' distribution of a species over time.
#' @param count If TRUE the returned value will be the count of non-zero cells
#' at the raster location (across timesteps).
