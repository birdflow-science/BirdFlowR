...
#' species range.
#' @param treat_na_as_zero  TRUE if NA's should be treated as zero FALSE
#' otherwise.
...
process_rasters <- function(res,
  ...
  # Overwrite NA with zero if appropriate
  # this is done so because often the NA represents very different habitat
  # than nearby non-na values so is ussually more appropriately treated as
  # non-habitat (zero) than not available.
  if (treat_na_as_zero) {
