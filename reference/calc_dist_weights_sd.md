# Standard deviation of the spread kernel used by [`calc_dist_weights()`](https://birdflow-science.github.io/BirdFlowR/reference/calc_dist_weights.md)
...
can plot the same spread it uses without duplicating the
kernel-selection logic.

## Usage

``` r
calc_dist_weights_sd(
  dist_along_line,
  line_lengths,
  res_m,
  kernel,
...
- dist_along_line:
  ...
  How far along the line the point of interest is (m)

- line_lengths:

  How long the line is (m)
...
A vector of standard deviations (m), the same length as
`dist_along_line`.
