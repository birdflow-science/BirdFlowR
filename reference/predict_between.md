...
you have information about their locations or distributions at two or
more points in time.
...
A matrix of dimensions `n_active(bf)` x `n_query_timesteps` of marginal
probability distributions p(x_t \| observations), one column per
timestep. Same format as
[`predict()`](https://birdflow-science.github.io/BirdFlowR/reference/predict.BirdFlow.md).
The log-likelihood of the observations is stored as attribute `log_z`.

## See also

[`predict()`](https://birdflow-science.github.io/BirdFlowR/reference/predict.BirdFlow.md)
for predictions from a single point in time,
[`route_between()`](https://birdflow-science.github.io/BirdFlowR/reference/route_between.md)
for creating routes from the same inputs as `predict_between()`.
