...
`calc_spherical_detection_rate()` and
...
`calc_spherical_detection_rate()` and logical with
[`is_between()`](https://birdflow-science.github.io/BirdFlowR/reference/is_between.md)
...
It's much slower and not recommended for production use, but is kept to
allow assessing the impact of switching from spherical to Euclidean
geometry.
