...
This function calculates a square, distance matrix with
[`n_active()`](https://birdflow-science.github.io/BirdFlowR/reference/dimensions.md)
cells on each side. Each cell represents the distance between its row
index and column index treated as locations in the state space; e.g. The
cell `[3, 8]` represents the distance between locations 3 and 8 in the
state space vector.
...
a n x n matrix containing pairwise distances in kilometers between every
active cell in the BirdFlow object.
