...
test_that("plot_distr() works with dynamic mask", {
  ...
  # 3. single plot
  expect_no_error(p3 <- plot_distr(get_distr(bf, 1), bf, show_dynamic_mask = TRUE) )
  expect_no_error(print(p3))

  # 4 facet
  expect_no_error(p4 <- plot_distr(get_distr(bf, 1:3), bf, show_dynamic_mask = TRUE) )
...
test_that("plot_distr() works with custom colors and custom value label and subset", {
